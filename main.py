...
# def generate_system_comparison_bar_chart(user):
#     # Fetch and prepare data from the Metrics table for all users and the specific user
#     user_stats = Metrics.query.filter_by(user_id=user['id']).all()
#     all_stats = Metrics.query.all()
    
#     # Example data processing
#     # Calculate averages or totals for system and user
#     # Visualization code as shown earlier, adjusted for comparative data
...
#     p.setFont("Helvetica", 12)
#     p.setTitle("User Report")

#     p.drawString(50, 750, "User Report")
...
#         p.drawString(50, y - 120, f"Total Carbon Offset: {user['total_carbon_offset']} tons")
#         p.drawString(50, y - 140, f"Carbon Footprint: {user['carbon_footprint']} tons")
#         if user['offset_under_neutral'] is not None:
#             p.drawString(50, y - 160, f"Offset Under Neutral: {user['offset_under_neutral']} tons")
#         else:
#             p.drawString(50, y - 160, "Offset Under Neutral: N/A")
#         y -= 150 

#         generate_carbon_footprint_comparison_graph(user['carbon_footprint'], user['country'])
#         p.drawImage("carbon_footprint_comparison1.png", 50, y - 260, width=400, height=200)

#         generate_carbon_footprint_distribution_pie_chart()
#         p.drawImage("carbon_footprint_distribution_pie_chart1.png", 50, y - 500, width=400, height=200)

#         y -= 480  # Adjust for next user
...
# def generate_carbon_footprint_distribution_pie_chart():
#     total_users = User.query.count()

#     negative_users = User.query.filter(User.offset_under_neutral  > 0).count()
#     positive_users = User.query.filter(User.offset_under_neutral  < 0).count()
#     neutral_users = User.query.filter(User.offset_under_neutral == None).count()
#     neutral_users2 = User.query.filter(User.offset_under_neutral == 0).count()

#     # Calculate percentages
#     percentage_positive = (positive_users / total_users) * 100
#     percentage_negative = (negative_users / total_users) * 100
#     percentage_neutral = (neutral_users+neutral_users2 / total_users) * 100

#     # Pie chart data
#     categories = ['Positive', 'Negative', 'Neutral']
#     contribution_values = [percentage_positive, percentage_negative, percentage_neutral]

#     # Plotting the pie chart
#     plt.figure(figsize=(8, 5))
#     plt.pie(contribution_values, labels=categories, autopct='%1.1f%%', startangle=140)

#     # Adding title
#     plt.title('Carbon Footprint Distribution Among Users')

#     # Save the pie chart
#     plt.savefig('carbon_footprint_distribution_pie_chart1.png')
...
def admin_dashboard():
    ...
    staffs = Staff().query.all()
    admins = Admin().query.all()
    return render_template('admin/dashboard.html',users=users,staffs=staffs,admins=admins)
...
def admin_allStaff():
    ...
    staffs = Staff().query.all()
    print(staffs)
    return render_template('admin/allStaff.html',staffs=staffs)
...
def admin_allAdmin():
    ...
    return render_template('admin/allAdmin.html',admins=admins)
...
def admin_addStaff():
    ...
    if request.method == 'POST':
        ...
        new_staff = Staff(email=email, password=bcrypt.generate_password_hash(password, 10).decode('utf-8'))

        db.session.add(new_staff)
        db.session.commit()
        return render_template("admin/addStaff.html", success_message="Staff has been created.", email=email, password=password)
    else:
...
def admin_addProduct():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return render_template("admin/addProduct.html", success_message="Product has been added")
...
def admin_addUser():
    ...
    if request.method == 'POST':
        ...
        return render_template('admin/addUser.html', success_message="User has been created.", email=email, password=password)
    else:
...
def admin_changeUserPwd(id):
    ...
    if request.method == 'GET':
        return render_template('admin/changeUserPwd.html',user=user)
    else:
        email = request.form['email']
        print("email: ",email)
        password = generate_random_password()
        ...
        return render_template('admin/changeUserPwd.html',success_message="User password has been reset.",email=email, password=password,user=user)
...
def admin_changeUserProfile(id):
    ...
    if request.method == 'POST':
        ...
        return render_template('admin/changeUserProfile.html', success_message="User profile Updated successfully.", user=user)
    else:
        return render_template('admin/changeUserProfile.html',user=user)
...
def admin_changeStaffProfile(id):
    ...
    if request.method == 'POST':
        ...
        return render_template('admin/changeStaffProfile.html', success_message="Staff has been updated.",email=email,staff=staff)
    else:
        return render_template('admin/changeStaffProfile.html',staff=staff)
...
def admin_changeStaffPwd(id):
    ...
    if request.method == 'POST':
        ...
        return render_template('admin/addAdmin.html', success_message="Staff has been updated.",email=email, password=password)
    else:
        return render_template('admin/changeStaffPwd.html',staff=staff)
